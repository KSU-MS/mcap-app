...
func (h *mcapHandler) ProcessMatlabJob(w http.ResponseWriter, r *http.Request) *HandlerError {
	...
	// MPS can be disabled at startup (for example in local/dev environments).
	// Guard submissions so this endpoint fails gracefully when no MPS client is configured.
	if h.mpsClient == nil {
		return NewHandlerError("MPS is disabled on this deployment", http.StatusServiceUnavailable)
	}
