...
func (s *SyncService) retrieveData(ctx context.Context, client *github.Client, latestHash string, repo string) error {
	...
	for _, asset := range release.Assets {
		...
		if regexAnalyzer.Match([]byte(*asset.Name)) {
			filePath := filepath.Join(filesBaseDir, repo, *asset.Name)
...
func Initializer(ctx context.Context, s3Repository *s3.S3Repository) *SyncService {
	...
	err := os.MkdirAll(filepath.Join(filesBaseDir, "HT_CAN"), 0755)
	if err != nil {
		log.Println("Error creating directories:", err)
	}

	err = os.MkdirAll(filepath.Join(filesBaseDir, "HT_proto"), 0755)
	if err != nil {
		log.Println("Error creating directories:", err)
	}
